...
const navItems = [
  ...
  { groupKey: 'salesOs.groups.products', route: 'products', labelKey: 'nav.products', icon: 'products', allowedRoles: ALL_ROLES, order: 210, activeRoutes: ['products', 'product-library-products', 'product-intelligence-center', 'product-knowledge-center', 'product-library'] },
  ...
  { groupKey: 'salesOs.groups.products', route: 'ai-knowledge-center', labelKey: 'nav.aiKnowledgeCenter', icon: 'document', allowedRoles: ALL_ROLES, requiredPermission: 'knowledge-dashboard', order: 280 },
...
const routeAliases = Object.freeze({ crm: 'sales-customers', 'new-inquiry': 'inquiries', 'product-library-products':'products', 'product-intelligence-center':'products', 'product-knowledge-center':'products', 'product-library':'products' });
...
function renderLanguageMenu() {
  $('#language-menu').innerHTML = getSupportedLocales().map(locale => `<button type="button" class="language-option ${locale.code === getLocale() ? 'is-active' : ''}" data-language="${locale.code}" data-no-translate><span>${esc(locale.name)}</span>${locale.code === getLocale() ? icon('check') : ''}</button>`).join('');
}
...
async function navigate(route, replace = false) {
  ...
  const renderers = {
    ...
    products: renderProductLibraryProducts,
    'product-library-products': renderProductLibraryProducts,
    'product-library-categories': renderProductLibraryCategoriesV2,
    ...
    'ai-knowledge-center': renderAiKnowledgeCenter,
...
async function renderLibraryProductDetailLegacy(id,tab='general'){
  ...
  $('#page').innerHTML=`${pageHeader(p.name,`${p.sku} · Product Library`,`<button class="button" data-route="products">Back to Products</button>`)}${tabbar}${content}`;
...
async function saveLibraryFoundation(id,f,changes={}){const body={};if(changes.attribute_values)body.attribute_values=changes.attribute_values;if(changes.variant_axis_attribute_ids)body.variant_axis_attribute_ids=changes.variant_axis_attribute_ids;if(changes.related_product_ids)body.related_product_ids=changes.related_product_ids;if(changes.frequently_bought_together_ids)body.frequently_bought_together_ids=changes.frequently_bought_together_ids;return api(`/api/products/${id}/foundation`,{method:'PUT',body:JSON.stringify(body)})}

async function renderLibraryProductDetailFoundationV1(id,tab='general'){
  if(!['variants','attributes','relationships','channel'].includes(tab))return renderLibraryProductDetailLegacy(id,tab);
  const data=await api(`/api/products/${id}`),p=data.product,f=data.foundation;state.productDetail=data;
  const tabs=[['general',t('productFinal.overview')],['images',t('productFinal.media')],['attributes',t('productFinal.specifications')],['variants',t('productFinal.variants')],['pricing',t('productFinal.pricing')],['relationships',t('productFinal.relationships')],['channel',t('productFinal.channelReadiness')]];
  const tabbar=`<div class="knowledge-tabs library-detail-tabs">${tabs.map(([key,label])=>`<button class="${tab===key?'is-active':''}" data-action="library-product-tab" data-id="${id}" data-tab="${key}">${label}</button>`).join('')}</div>`;
  if(tab==='attributes'){
    const axes=new Set(f.variantAxes.map(axis=>Number(axis.attribute_id))),axisCandidates=f.attributeDefinitions.filter(attribute=>attribute.category_config?.can_be_variant_axis);
    const fields=f.attributeDefinitions.filter(attribute=>attribute.category_config?.show_on_product!==0).map(attribute=>productAttributeEditor(attribute,f.attributeValues.find(value=>value.attribute_id===attribute.id&&!value.variant_id)?.value||'')).join('');
    $('#page').innerHTML=`${pageHeader(p.name,`${p.sku} · ${t('productFoundation.products')}`,`<button class="button" data-route="products">Back to Products</button>`)}${tabbar}<article class="panel"><div class="panel-title"><h2>${t('productFoundation.fixedAttributes')}</h2><p>${t('productFoundation.attributesHelp')}</p></div><form id="detail-attributes-form" class="foundation-form"><div class="attribute-value-grid">${fields||`<p>${t('productFoundation.noAttributes')}</p>`}</div><fieldset><legend>${t('productFoundation.variantAxes')}</legend><p class="form-help">${t('productFoundation.variantsHelp')}</p><div class="axis-choice-grid">${axisCandidates.map(attribute=>`<label class="check-field"><input type="checkbox" data-axis-id="${attribute.id}" ${axes.has(attribute.id)?'checked':''}><span>${esc(getLocale()==='zh-CN'?(attribute.name_zh||attribute.name):(attribute.name_en||attribute.name))}</span></label>`).join('')||`<p>${t('productFoundation.noAttributes')}</p>`}</div>${f.variantAxisWarning?`<p class="form-warning">${t('productFoundation.axisWarning')}</p>`:''}</fieldset><button class="button button--primary">${t('productFoundation.saveAttributes')}</button></form></article>`;
    $('#detail-attributes-form').addEventListener('submit',async event=>{event.preventDefault();const form=event.currentTarget,values=[...form.querySelectorAll('[data-create-attribute]')].map(input=>({attribute_id:Number(input.dataset.attributeId),value:input.type==='checkbox'?String(input.checked):input.multiple?[...input.selectedOptions].map(option=>option.value).join(', '):input.value})),variant_axis_attribute_ids=[...form.querySelectorAll('[data-axis-id]:checked')].map(input=>Number(input.dataset.axisId));await saveLibraryFoundation(id,f,{attribute_values:values,variant_axis_attribute_ids});await renderLibraryProductDetail(id,'attributes')});return;
  }
  const axisFields=variantAxisFields(f.variantAxes),optionText=variant=>(variant.option_values||[]).map(option=>`${getLocale()==='zh-CN'?(option.name_zh||option.name):(option.name_en||option.name)}: ${option.value}`).join(' · '),canViewSensitive=Boolean(data.capabilities?.canViewSensitive??state.products?.capabilities?.canViewSensitive);
  $('#page').innerHTML=`${pageHeader(p.name,`${p.sku} · ${t('productFoundation.products')}`,`<button class="button" data-route="products">Back to Products</button>`)}${tabbar}<article class="panel"><div class="panel-title"><h2>${t('productFoundation.variants')}</h2><p>${t('productFoundation.variantsHelp')}</p></div>${f.variantAxisWarning?`<p class="form-warning">${t('productFoundation.axisWarning')}</p>`:''}<div class="table-scroll"><table class="data-table"><thead><tr><th>${t('productFoundation.variantName')}</th><th>SKU</th><th>${t('productFoundation.variantOptions')}</th><th>${t('productFoundation.salesPriceOverride')}</th><th>${t('fields.status')}</th><th></th></tr></thead><tbody>${f.variants.map(variant=>`<tr><td><strong>${esc(variant.variant_name)}</strong></td><td>${esc(variant.variant_sku||'—')}</td><td>${esc(optionText(variant)||'—')}</td><td>${variant.reference_price==null?t('productFoundation.inherited'):quoteMoney(variant.reference_price)}</td><td>${badge(variant.status)}</td><td><button class="button button--compact" data-action="library-edit-variant" data-product-id="${id}" data-id="${variant.id}" data-name="${esc(variant.variant_name)}" data-price="${variant.reference_price??''}" data-sku="${esc(variant.variant_sku||'')}" data-status="${esc(variant.status)}">${t('common.edit')}</button></td></tr>`).join('')||`<tr><td colspan="6">${t('productFoundation.noVariants')}</td></tr>`}</tbody></table></div><form id="detail-variant-create" class="foundation-form inline-create-form"><h3>${t('productFoundation.newVariant')}</h3><div class="form-grid"><label class="field"><span>${t('productFoundation.variantName')}</span><input name="variant_name" required></label><label class="field"><span>SKU</span><input name="variant_sku"><small class="form-help">${t('productFoundation.skuHelp')}</small></label>${axisFields}<label class="field"><span>${t('productFoundation.salesPriceOverride')}</span><input name="reference_price" type="number" step=".01" placeholder="${t('productFoundation.inherited')}"></label><label class="field"><span>${t('fields.status')}</span><select name="status"><option>Active</option><option>Hidden</option><option>Coming Soon</option><option>Discontinued</option></select></label></div>${variantPimFields({},canViewSensitive)}<button class="button button--primary">${t('productFoundation.create')}</button></form></article>`;
  $('#detail-variant-create').addEventListener('submit',async event=>{event.preventDefault();const form=event.currentTarget,body=Object.fromEntries(new FormData(form));body.option_values=[...form.querySelectorAll('[data-variant-axis]')].map(input=>({attribute_id:Number(input.dataset.attributeId),value:input.value})).filter(option=>option.value);await api(`/api/products/${id}/variants`,{method:'POST',body:JSON.stringify(body)});await renderLibraryProductDetail(id,'variants')});
}

function productDetailTabsV2(id,active){const tabs=[['general','productFinal.overview'],['images','productFinal.media'],['attributes','productFinal.specifications'],['variants','productFinal.variants'],['pricing','productFinal.pricing'],['relationships','productFinal.relationships'],['channel','productFinal.channelReadiness']];return `<div class="knowledge-tabs library-detail-tabs">${tabs.map(([key,label])=>`<button class="${active===key?'is-active':''}" data-action="library-product-tab" data-id="${id}" data-tab="${key}">${t(label)}</button>`).join('')}</div>`}
...
async function renderLibraryProductDetail(id,tab='general'){
  if(!['attributes','relationships','channel'].includes(tab)){
    ...
    document.querySelectorAll('.product-image-preview img,.library-product-hero img').forEach(image=>image.addEventListener('error',()=>{image.replaceWith(Object.assign(document.createElement('span'),{className:'library-image-empty',textContent:t('productFinal.imageUnavailable')}))},{once:true}));
    return;
  }
  const data=await api(`/api/products/${id}`),p=data.product,f=data.foundation;state.productDetail=data;
  const header=`${pageHeader(p.name,`${p.sku} · ${t('productFoundation.products')}`,`<button class="button" data-route="products">${t('productFinal.backProducts')}</button>`)}${productDetailTabsV2(id,tab)}`;
  if(tab==='attributes'){
    const activeAxis=Number(f.variantAxes[0]?.attribute_id||0),candidates=f.attributeDefinitions.filter(attribute=>attribute.category_config?.can_be_variant_axis),values=new Map(f.attributeValues.filter(value=>!value.variant_id).map(value=>[Number(value.attribute_id),value.value])),obsolete=f.attributeValues.filter(value=>!value.variant_id&&!value.applicable);
    $('#page').innerHTML=`${header}<article class="panel"><div class="panel-title"><h2>${t('productFinal.specifications')}</h2><p>${t('productFoundation.attributesHelp')}</p></div><form id="product-specifications-v2" class="foundation-form"><div class="attribute-value-grid">${f.attributeDefinitions.map(attribute=>productAttributeEditor(attribute,values.get(attribute.id)||'')).join('')||`<p>${t('productFoundation.noAttributes')}</p>`}</div>${obsolete.length?`<aside class="form-warning obsolete-attributes"><strong>${t('productFinal.inapplicableAttributes')}</strong>${obsolete.map(value=>`<span>${esc(getLocale()==='zh-CN'?(value.name_zh||value.name):(value.name_en||value.name))}: ${esc(value.value)}</span>`).join('')}</aside>`:''}<fieldset><legend>${t('productFinal.variantOption')}</legend><p class="form-help">${t('productFinal.oneVariantHelp')}</p><div class="axis-choice-grid"><label class="check-field"><input type="radio" name="variant_axis" value="0" ${activeAxis?'':'checked'}><span>${t('productFinal.noVariant')}</span></label>${candidates.map(attribute=>`<label class="check-field"><input type="radio" name="variant_axis" value="${attribute.id}" ${activeAxis===attribute.id?'checked':''}><span>${esc(getLocale()==='zh-CN'?(attribute.name_zh||attribute.name):(attribute.name_en||attribute.name))}</span></label>`).join('')}</div></fieldset><button class="button button--primary">${t('common.save')}</button></form></article>`;
    $('#product-specifications-v2').addEventListener('submit',async event=>{event.preventDefault();const form=event.currentTarget,attribute_values=[...form.querySelectorAll('[data-create-attribute]')].map(input=>({attribute_id:Number(input.dataset.attributeId),value:input.type==='checkbox'?String(input.checked):input.multiple?[...input.selectedOptions].map(option=>option.value).join(', '):input.value})),axis=Number(new FormData(form).get('variant_axis'));await api(`/api/products/${id}/foundation`,{method:'PUT',body:JSON.stringify({attribute_values,variant_axis_attribute_ids:axis?[axis]:[]})});await renderLibraryProductDetail(id,'attributes')});return;
  }
  if(tab==='relationships'){
    const selected=new Map((f.relationships||[]).map(item=>[Number(item.id),item.relationship_type]));
    $('#page').innerHTML=`${header}<article class="panel"><div class="panel-title"><h2>${t('productFinal.relationships')}</h2><p>${t('productFinal.relationshipsHelp')}</p></div><form id="product-relationships-v2" class="relationship-list">${data.options.products.map(product=>`<label><input type="checkbox" data-relationship-product="${product.id}" ${selected.has(product.id)?'checked':''}><span>${esc(product.name)} · ${esc(product.sku)}</span><select data-relationship-type>${[['complementary','productFinal.complementary'],['alternative','productFinal.alternative'],['frequently_bought_together','productFinal.frequentlyBought'],['upgrade','productFinal.upgrade']].map(([value,key])=>`<option value="${value}" ${selected.get(product.id)===value?'selected':''}>${t(key)}</option>`).join('')}</select></label>`).join('')}<button class="button button--primary">${t('common.save')}</button></form></article>`;
    $('#product-relationships-v2').addEventListener('submit',async event=>{event.preventDefault();const relationships=[...event.currentTarget.querySelectorAll('[data-relationship-product]:checked')].map(input=>({target_product_id:Number(input.dataset.relationshipProduct),relationship_type:input.closest('label').querySelector('[data-relationship-type]').value}));await api(`/api/products/${id}/foundation`,{method:'PUT',body:JSON.stringify({relationships})});await renderLibraryProductDetail(id,'relationships')});return;
  }
  const ready=['ready','published'].includes(p.publish_status);
  $('#page').innerHTML=`${header}<article class="panel channel-readiness"><div class="panel-header"><div class="panel-title"><h2>${t('productFinal.channelReadiness')}</h2><p>${t('productFinal.channelHelp')}</p></div>${badge(ready?t('productFinal.ready'):t('productFinal.draft'))}</div><dl class="library-field-grid"><div><dt>${t('productFinal.salesMode')}</dt><dd>${esc(p.sales_mode||'quote_only')}</dd></div><div><dt>${t('productFinal.publishStatus')}</dt><dd>${esc(p.publish_status||'draft')}</dd></div><div><dt>${t('productFinal.priceDisplay')}</dt><dd>${esc(p.price_display_mode||'request_quote')}</dd></div><div><dt>${t('productFinal.externalSync')}</dt><dd>${t('productFinal.notConnected')}</dd></div></dl><p class="form-help">${t('productFinal.noRealChannel')}</p></article>`;
}

function variantPimFields(record={},canViewSensitive=Boolean(state.products?.capabilities?.canViewSensitive)){const field=(name,label,type='text',step='',help='')=>`<label class="field"><span>${label}${help?` <small title="${help}">?</small>`:''}</span><input name="${name}" type="${type}" ${step?`step="${step}"`:''} value="${esc(record[name]??'')}"></label>`,supplier=canViewSensitive?`${field('default_supplier','Default Supplier')}${field('supplier_sku','Supplier SKU')}${field('supplier_cost','Supplier Cost','number','.01')}${field('supplier_lead_time_days','Supplier Lead Time (days)','number')}${field('supplier_moq','Supplier MOQ','number','.01',t('productFoundation.moqHelp'))}<label class="field"><span>Supplier Notes</span><textarea name="supplier_notes">${esc(record.supplier_notes||'')}</textarea></label>`:'';return `<details class="variant-pim-fields"><summary>${t('productFoundation.overrideDefaults')}</summary><p class="form-help">${t('productFoundation.overrideHelp')}</p><div class="form-grid">${field('material',t('fields.material'))}${field('finish',t('intelligence.finish'))}${field('color',t('intelligence.color'))}${field('moq','MOQ','number','.01',t('productFoundation.moqHelp'))}${field('lead_time_days',t('fields.leadTime'),'number')}${field('cbm','CBM','number','.0001',t('productFoundation.cbmHelp'))}${field('gross_weight_kg','Gross Weight (kg)','number','.001')}${field('net_weight_kg','Net Weight (kg)','number','.001')}${field('packing_info','Packing Information')}${supplier}</div></details>`}
...
async function saveProductForm(event) {
  ...
  payload.request_quote_enabled=Boolean(form.elements.request_quote_enabled?.checked);
  payload.customization_available=Boolean(form.elements.customization_available?.checked);
  const current=state.products.products.find(item=>item.id===Number(form.dataset.id));
  if(current&&Number(payload.category_id)!==Number(current.category_id)&&!window.confirm(t('productFoundation.categoryChangeWarning')))return;
  ...
  try {
    ...
    await api(`/api/products/${saved.product.id}/foundation`,{method:'PUT',body:JSON.stringify({attribute_values})});
...
async function saveProductImage(event) {
  ...
  try {
    ...
    if(state.route==='products')await renderLibraryProductDetail(form.dataset.productId,'images');else await renderProductDetail(form.dataset.productId);
  } catch (error) {
...
async function markMainImage(mediaId) {
  ...
  toast(t('intelligence.imageSaved'));
  if(state.route==='products')await renderLibraryProductDetail(productId,'images');else await renderProductDetail(productId);
}
...
function productIntelligenceDashboard(products) {
  ...
  return `<section class="pic-dashboard">
    <div class="pic-hero panel">
      ...
      <div class="pic-hero-actions"></div>
    </div>
...
async function renderProductLibraryProducts(){
  ...
  $('#page').innerHTML = `${pageHeader(t('productFoundation.productsTitle'),t('productFoundation.productsSubtitle'),`<button class="button button--primary" data-action="add-product">${icon('plus')} ${t('productFoundation.newProduct')}</button>`)}
    ${productIntelligenceDashboard(products)}
    <article class="panel pic-product-list">
      <div class="panel-header"><div class="panel-title"><h2>${t('productFoundation.products')}</h2><p>${t('productFoundation.productListHelp')}</p></div><span class="pic-source-note">${esc(productIntelligence.source || 'Product Library')}</span></div>
...
function variantAxisFields(axes=[],values=[]){const selected=new Map(values.map(item=>[Number(item.attribute_id),item.value]));return axes.length?`<section class="variant-axis-fields"><h3>${t('productFoundation.variantOptions')}</h3>${axes.map(axis=>`<label class="field"><span>${esc(getLocale()==='zh-CN'?(axis.name_zh||axis.name):(axis.name_en||axis.name))}${axis.unit?` (${esc(axis.unit)})`:''}</span><input data-axis-attribute="${axis.attribute_id}" value="${esc(selected.get(Number(axis.attribute_id))||'')}" required></label>`).join('')}</section>`:`<p class="form-help">${t('productFoundation.noAttributes')}</p>`}
function refreshVariantAxes(form,products){const product=products.find(item=>item.id===Number(form.elements.product_id.value));form.querySelector('[data-variant-axis-container]').innerHTML=variantAxisFields(product?.variant_axes||[])}
async function renderProductLibraryVariants(){
  const data=await api('/api/product-variants');
  $('#page').innerHTML=`${pageHeader(t('productFoundation.variants'),t('productFoundation.variantsHelp'))}<section class="library-management-grid"><article class="panel"><h2>${t('productFoundation.newVariant')}</h2><form id="library-variant-create" class="foundation-form"><label class="field"><span>${t('productFoundation.parentProduct')}</span><select name="product_id" required>${data.products.map(p=>`<option value="${p.id}">${esc(p.name)} · ${esc(p.sku)}</option>`).join('')}</select></label><div data-variant-axis-container></div><label class="field"><span>${t('productFoundation.variantName')}</span><input name="variant_name" required></label><label class="field"><span>SKU <small title="${t('productFoundation.skuHelp')}">?</small></span><input name="variant_sku"></label><label class="field"><span>${t('productFoundation.salesPriceOverride')}</span><input name="reference_price" type="number" step=".01"></label><label class="field"><span>${t('productFoundation.status')}</span><select name="status"><option>Active</option><option>Hidden</option><option>Coming Soon</option><option>Discontinued</option></select></label>${variantPimFields({},data.capabilities.canViewSensitive)}<button class="button button--primary">${t('productFoundation.create')}</button></form></article><article class="panel"><h2>${t('productFoundation.allVariants')}</h2><div class="table-scroll"><table class="data-table"><thead><tr><th>${t('productFoundation.parentProduct')}</th><th>${t('productFoundation.variantName')}</th><th>SKU</th><th>${t('productFoundation.variantOptions')}</th><th>${t('productFoundation.salesPriceOverride')}</th><th>${t('productFoundation.status')}</th><th>${t('productFoundation.actions')}</th></tr></thead><tbody>${data.variants.map(v=>`<tr><td>${esc(v.product_name)}</td><td><strong>${esc(v.variant_name)}</strong></td><td>${esc(v.variant_sku||'—')}</td><td>${v.option_values.map(option=>esc(option.value)).join(' · ')||t('productFoundation.inherited')}</td><td>${v.reference_price==null?t('productFoundation.inherited'):quoteMoney(v.reference_price)}</td><td>${badge(v.status)}</td><td><button class="button button--compact" data-action="library-edit-variant" data-product-id="${v.product_id}" data-id="${v.id}" data-name="${esc(v.variant_name)}" data-price="${v.reference_price??''}" data-sku="${esc(v.variant_sku||'')}" data-status="${esc(v.status)}">${t('productFoundation.edit')}</button></td></tr>`).join('')||`<tr><td colspan="7">${t('productFoundation.noVariants')}</td></tr>`}</tbody></table></div></article></section>`;
  const form=$('#library-variant-create');refreshVariantAxes(form,data.products);form.elements.product_id.addEventListener('change',()=>refreshVariantAxes(form,data.products));form.addEventListener('submit',async event=>{event.preventDefault();const body=Object.fromEntries(new FormData(form)),productId=body.product_id;delete body.product_id;body.option_values=[...form.querySelectorAll('[data-axis-attribute]')].map(input=>({attribute_id:Number(input.dataset.axisAttribute),value:input.value}));await api(`/api/products/${productId}/variants`,{method:'POST',body:JSON.stringify(body)});await renderProductLibraryVariants()});
}
...
async function renderProductLibraryTags(){const data=await api('/api/product-tags'),groupOptions=data.groups.map(group=>`<option>${esc(group)}</option>`).join('');$('#page').innerHTML=`${pageHeader(t('productFoundation.tags'),t('productFoundation.tagsHelp'))}<section class="library-management-grid"><article class="panel"><h2>${t('productFoundation.newTag')}</h2><form id="library-tag-create" class="foundation-form"><label class="field"><span>${t('fields.name')}</span><input name="tag_name" required></label><label class="field"><span>${t('fields.code')}</span><input name="code" required></label><label class="field"><span>${t('fields.group')}</span><select name="tag_type">${groupOptions}</select></label><label class="field"><span>${t('productFoundation.sortOrder')}</span><input name="sort_order" type="number" value="0"></label><label class="check-field"><input name="active" type="checkbox" checked><span>${t('productFoundation.active')}</span></label><button class="button button--primary">${t('productFoundation.create')}</button></form></article><article class="panel"><h2>${t('productFoundation.tags')}</h2><div class="foundation-manager-list">${data.tags.map(tag=>`<article><span><strong>${esc(tag.tag_name)}</strong><small>${esc(tag.tag_type)} · ${tag.usage_count} ${t('productFoundation.products')} · ${t(tag.active?'productFoundation.active':'productFoundation.disabled')}</small></span><div><button class="button button--compact" data-action="library-edit-tag" data-id="${tag.id}" data-name="${esc(tag.tag_name)}" data-code="${esc(tag.code)}" data-group="${esc(tag.tag_type)}" data-sort="${tag.sort_order||0}">${t('productFoundation.edit')}</button><button class="button button--compact" data-action="library-toggle-tag" data-id="${tag.id}" data-active="${tag.active?1:0}">${t(tag.active?'productFoundation.disabled':'productFoundation.active')}</button></div></article>`).join('')}</div></article></section>`;$('#library-tag-create').addEventListener('submit',async event=>{event.preventDefault();const form=event.currentTarget,body=Object.fromEntries(new FormData(form));body.active=form.active.checked;await api('/api/product-tags',{method:'POST',body:JSON.stringify(body)});await renderProductLibraryTags()})}
...
function productAttributeTypeOptions(){return ['Text','Number','Select','Multi-select','Boolean','Color','Dimension','Date'].map(type=>`<option value="${type}">${t(`productFoundation.type${type.replace('-','')}`)}</option>`).join('')}

function attributeMasterForm(categories){return `<div class="field-row"><label class="field"><span>${t('productFoundation.nameEn')}</span><input name="name_en" required></label><label class="field"><span>${t('productFoundation.nameZh')}</span><input name="name_zh" required></label></div><label class="field"><span>${t('fields.code')}</span><input name="code" readonly placeholder="${t('productFoundation.generatedFromEnglish')}"><small class="form-help">${t('productFoundation.codeHelp')}</small></label><label class="field"><span>${t('productFinal.aliases')}</span><textarea name="aliases" placeholder="${t('productFinal.aliasPlaceholder')}"></textarea><small class="form-help">${t('productFinal.aliasHelp')}</small></label><label class="field"><span>${t('productFoundation.applicableCategories')}</span><select name="category_ids" multiple size="5" required>${categories.filter(category=>category.active).map(category=>`<option value="${category.id}">${esc(category.name)}</option>`).join('')}</select><small class="form-help">${t('productFoundation.categorySpecificHelp')}</small></label><label class="field"><span>${t('fields.dataType')}</span><select name="data_type">${productAttributeTypeOptions()}</select></label><label class="field"><span>${t('fields.options')}</span><textarea name="options"></textarea></label><div class="field-row"><label class="field"><span>${t('fields.unit')}</span><input name="unit"></label><label class="field"><span>${t('productFoundation.minimum')}</span><input name="minimum_value" type="number" step="any"></label><label class="field"><span>${t('productFoundation.maximum')}</span><input name="maximum_value" type="number" step="any"></label><label class="field"><span>${t('fields.sortOrder')}</span><input name="sort_order" type="number" value="0"></label></div><fieldset><legend>${t('fields.display')}</legend>${[['show_in_library','productFoundation.productLibrary',true],['show_on_website','productFoundation.website'],['show_in_quote','productFoundation.quote'],['show_in_pi','productFoundation.pi'],['internal_only','productFoundation.internalOnly']].map(([name,key,checked])=>`<label class="check-field"><input type="checkbox" name="${name}" ${checked?'checked':''}> <span>${t(key)}</span></label>`).join('')}</fieldset><label class="check-field"><input name="active" type="checkbox" checked> <span>${t('productFoundation.active')}</span></label>`}

async function renderProductLibraryAttributes(){const [data,categoryData]=await Promise.all([api('/api/product-attributes'),api('/api/product-categories')]);const categoryName=id=>categoryData.categories.find(category=>category.id===Number(id))?.name,displayName=attribute=>getLocale()==='zh-CN'?(attribute.name_zh||attribute.name):(attribute.name_en||attribute.name);$('#page').innerHTML=`${pageHeader(t('productFoundation.attributesTitle'),t('productFoundation.attributesSubtitle'))}<section class="library-management-grid"><article class="panel"><h2>${t('productFoundation.newAttribute')}</h2><form id="library-attribute-create" class="foundation-form">${attributeMasterForm(categoryData.categories)}<button class="button button--primary">${t('productFoundation.createAttribute')}</button></form></article><article class="panel"><h2>${t('productFoundation.attributesTitle')}</h2><div class="foundation-manager-list">${data.attributes.map(attribute=>`<article><span><strong>${esc(displayName(attribute))}</strong><small>${esc(attribute.name_en)} / ${esc(attribute.name_zh)} · <code>${esc(attribute.code)}</code> · ${esc(attribute.category_ids.map(categoryName).filter(Boolean).join(', ')||t('fields.allCategories'))} · ${esc(attribute.data_type)} · ${attribute.active?t('productFoundation.active'):t('productFoundation.disabled')}${attribute.used_count?` · ${t('productFoundation.usedBy')} ${attribute.used_count}`:''}</small></span><div><button class="button button--compact" data-action="library-edit-attribute" data-id="${attribute.id}">${t('common.edit')}</button></div></article>`).join('')||`<p>${t('productFoundation.noAttributes')}</p>`}</div></article></section>`;const form=$('#library-attribute-create');form.name_en.addEventListener('input',()=>{form.code.value=attributeCodePreview(form.name_en.value)});form.addEventListener('submit',async event=>{event.preventDefault();const body=Object.fromEntries(new FormData(form));body.name=body.name_en;body.category_ids=[...form.category_ids.selectedOptions].map(option=>Number(option.value));body.options=String(body.options||'').split(/\r?\n|,/).map(value=>value.trim()).filter(Boolean);for(const name of ['show_in_library','show_on_website','show_in_quote','show_in_pi','internal_only','active'])body[name]=form.elements[name].checked;await api('/api/product-attributes',{method:'POST',body:JSON.stringify(body)});await renderProductLibraryAttributes()})}
...
function importReviewRow(draft,data){const p=draft.mapped_product||{},imageSource=p.image_source||draft.image_status||'Supplier image evidence pending',conflict=p.auto_combination_blocked,candidates=Array.isArray(p.variant_axis_candidates)?p.variant_axis_candidates:[];const resolution=conflict?`<select name="resolution_action" required><option value="">Select one Variant Option</option>${candidates.map(value=>`<option value="variant_axis:${esc(value)}">${esc(value)}</option>`).join('')}<option value="split">Split into Products</option></select>`:draft.possible_match_product_id?`<select name="resolution_action"><option value="create_new">Create Product</option><option value="update_existing">Update Existing</option><option value="add_variant">Add Variant</option><option value="ignore">Ignore</option></select>`:'<input type="hidden" name="resolution_action" value="create_new">';return `<tr data-import-draft="${draft.id}"><td><label class="import-select-cell"><input type="checkbox" data-import-select value="${draft.id}" ${draft.status==='Imported'||draft.status==='Rejected'?'disabled':''}></label></td><td class="import-product-image-cell">${draft.main_image_url?`<img src="${esc(draft.main_image_url)}" alt="Imported product image">`:'<div class="library-image-empty">Image Assets Needed</div>'}<small>${esc(imageSource)}</small><small>Image confidence: ${Number(p.image_confidence||draft.image_matching_confidence||0)}%</small></td><td class="import-product-info-cell"><input name="product_name" value="${esc(draft.product_name||'')}" aria-label="Product Name"><input name="product_sku" value="${esc(draft.product_sku||'')}" aria-label="SKU"><small>${data.capabilities.canViewSensitive?esc(p.default_supplier||'Supplier not set'):'Supplier hidden by role'}</small></td><td>${importCategorySelect(draft,data)}</td><td>${importAttributeSummary(draft)}</td><td>${importVariantSummary(draft)}</td><td>${importPricingCell(draft,data)}</td><td>${importQualityScore(draft)}${importIssueChips(draft)}</td><td>${badge(draft.status)}${resolution}</td><td><div class="row-actions vertical">${draft.suggested_variants.length>1?`<button class="button button--compact" data-action="split-import-draft" data-id="${draft.id}">Split</button>`:''}<button class="button button--compact" data-action="save-import-draft" data-id="${draft.id}">Save</button>${data.capabilities.canApprove&&draft.status!=='Imported'?`<button class="button button--compact button--primary" data-action="approve-import-draft" data-id="${draft.id}">Approve</button><button class="button button--compact" data-action="reject-import-draft" data-id="${draft.id}">Reject</button>`:''}</div></td></tr>`}
...
renderImports=async function(batchId=null){await renderImportsPIC2CBase(batchId);enhanceImportReviewPIC2C();document.querySelectorAll('.import-raw-debug,pre').forEach(node=>node.remove());const steps=[t('productFinal.importUpload'),t('productFinal.importMapping'),t('productFinal.importDetection'),t('productFinal.importReview'),t('productFinal.importConfirm')];document.querySelectorAll('.import-steps span').forEach((node,index)=>{node.textContent=steps[index]||''});const title=$('.page-header h1');if(title)title.textContent=t('productFinal.importCenter');const subtitle=$('.page-header p');if(subtitle)subtitle.textContent=t('productFinal.importHelp')};
...
async function handleAction(action, node) {
  if (action === 'logout') {
    ...
    if(!window.confirm(`Delete ${node.dataset.name}? Historical quotes and orders will be preserved.`))return;const result=await api(`/api/products/${node.dataset.id}`,{method:'DELETE'});toast(result.message||'Product deleted');state.products=null;await navigate('products',true);
