...
const routeAliases = Object.freeze({ crm: 'sales-customers', 'new-inquiry': 'inquiries', products:'product-library-products', 'product-intelligence-center':'product-library-products', 'product-knowledge-center':'product-library-products', 'product-library':'product-library-products' });
...
async function renderLibraryProductDetail(id,tab='general'){
  ...
  const data=await api(`/api/products/${id}`),p=data.product,f=data.foundation;state.productDetail=data;
  const header=`${pageHeader(p.name,`${p.sku} · ${t('productFoundation.products')}`,`<button class="button" data-route="product-library-products">${t('productFinal.backProducts')}</button>`)}${productDetailTabsV2(id,tab)}`;
  if(tab==='attributes'){
