...
async function submitProductImport(event){event.preventDefault();const form=event.currentTarget,file=form.elements.spreadsheet.files[0];if(!file)return;const bytes=new Uint8Array(await file.arrayBuffer());let binary='';for(let index=0;index<bytes.length;index+=0x8000)binary+=String.fromCharCode(...bytes.subarray(index,index+0x8000));const body=Object.fromEntries(new FormData(form));delete body.spreadsheet;body.filename=file.name;body.file_base64=btoa(binary);try{const result=await api('/api/imports/analyze',{method:'POST',body:JSON.stringify(body)});toast('Spreadsheet analyzed. Review drafts before approval.');await renderImports(result.batch.id)}catch(error){$('#import-error').textContent=error.message}}
...
previewQuote=function(){
  ...
  const description=item=>[item.name,item.size,item.display_material,item.display_finish,item.display_color].filter((value,index,array)=>value&&array.indexOf(value)===index);
  table.classList.add('pi-product-table-final');
  table.innerHTML=`<thead><tr><th>Product Image</th><th>SKU</th><th>Description</th><th>Qty</th><th>Unit Price</th><th>Amount</th></tr></thead><tbody>${q.all_items.map(item=>`<tr><td class="pi-product-image">${item.image_url?`<img src="${esc(item.image_url)}" alt="${esc(item.name)}">`:'<span>Reference image pending</span>'}</td><td>${esc(item.sku||'CUSTOM')}</td><td class="pi-short-description">${description(item).map((line,index)=>index?`<span>${esc(line)}</span>`:`<strong>${esc(line)}</strong>`).join('')}${item.swatch_image_url?`<span class="pi-swatch"><img src="${esc(item.swatch_image_url)}" alt="Approved finish color swatch">${esc([item.display_finish,item.display_color].filter(Boolean).join(' / '))}</span>`:''}${item.customer_remark?`<small>${esc(item.customer_remark)}</small>`:''}</td><td>${item.quantity}</td><td>${format(item.unit_price)}</td><td><strong>${format(item.line_total)}</strong></td></tr>`).join('')}</tbody>`;
