...
<article class="panel section-gap">${panelHeader('Version History','No data loss')}<div class="version-list">${quote.versions.map(v=>`<button class="button" data-action="view-quote-version" data-id="${id}" data-version="${v.version_number}">Version ${v.version_number}</button>`).join('')}</div></article>`;
  ...
  const syncPayment=(source)=>{const primary=source==='deposit'?depositInput:balanceInput;const secondary=source==='deposit'?balanceInput:depositInput;const value=Math.min(100,Math.max(0,Number(primary.value||0)));primary.value=value;secondary.value=100-value;if(!paymentNoteEdited)noteInput.value=`${depositInput.value}% deposit before production. ${balanceInput.value}% balance before shipment.`;const grand=Number(s.grand_total||0);$('[data-payment-amount="deposit"]').textContent=money(grand*Number(depositInput.value)/100);$('[data-payment-amount="balance"]').textContent=money(grand*Number(balanceInput.value)/100)};
  depositInput.addEventListener('input',()=>syncPayment('deposit'));balanceInput.addEventListener('input',()=>syncPayment('balance'));
}
function quoteBuilderPayload(){const form=$('#quote-builder-form');const b=Object.fromEntries(new FormData(form));b.items=[...document.querySelectorAll('[data-quote-item]')].map(row=>({id:Number(row.dataset.quoteItem),...Object.fromEntries([...row.querySelectorAll('[data-item-field]')].map(x=>[x.dataset.itemField,x.value]))}));b.custom_items=[...document.querySelectorAll('[data-custom-item]')].map(row=>({id:Number(row.dataset.customItem),...Object.fromEntries([...row.querySelectorAll('[data-custom-field]')].map(x=>[x.dataset.customField,x.value]))}));return b}
...
function previewQuote(){const q=state.salesQuote;const s=q.summary;const tbc=v=>v==null?'TBC':Number(v).toFixed(2);const modal=document.createElement('div');modal.className='modal-backdrop';modal.id='pi-preview';modal.innerHTML=`<div class="modal-card pi-preview"><button class="icon-button modal-close" data-action="close-pi-preview">${icon('close')}</button><header><img src="/favicon.svg" alt="Restaurant Setup Pro logo"><strong>RESTAURANT SETUP PRO</strong><small>Commercial Restaurant Furniture · Project Furniture Packages</small><h1>PROFORMA INVOICE</h1><p>${esc(q.quote_number)}</p></header><section class="pi-customer"><strong>Bill To</strong><h2>${esc(q.customer_name)}</h2><p>${esc(q.company||'')} · ${esc(q.country||'')}</p><p>Quote Date: ${esc(q.quote_date||'')} · Valid Until: ${esc(q.valid_until||'')}</p></section><table><thead><tr><th>Image</th><th>Product</th><th>Qty</th><th>Unit Price</th><th>Total</th></tr></thead><tbody>${q.all_items.map(i=>`<tr><td class="pi-product-image">${i.image_url?`<img src="${esc(i.image_url)}" alt="${esc(i.name)}">`:'<span>No image</span>'}</td><td><strong>${esc(i.name)}</strong><small>${esc(i.sku||'CUSTOM')} · ${esc(i.materials||'—')} · ${esc(i.size||'—')}</small><small>CBM ${tbc(i.cbm)} · GW ${tbc(i.gross_weight_kg)} · NW ${tbc(i.net_weight_kg)}</small></td><td>${i.quantity}</td><td>${money(i.unit_price)}</td><td><strong>${money(i.line_total)}</strong></td></tr>`).join('')}</tbody></table><section class="pi-summary"><div><span>Grand Total</span><strong>${money(s.grand_total)}</strong></div><p><b>Deposit ${Number(q.deposit_percent||0)}%</b> ${money(s.deposit_amount)} · <b>Balance ${Number(q.balance_percent||0)}%</b> ${money(s.balance_amount)}</p><p>Freight: ${s.freight_cost==null?'Freight To Be Quoted':money(s.freight_cost)}</p><p>${esc(q.payment_note||'')}</p><p>${esc(q.other_remark||'')}</p></section><footer>Authorized Signature ____________________</footer></div>`;document.body.append(modal)}
...
async function handleAction(action, node) {
  ...
  } else if(action==='preview-sales-quote'){previewQuote();
  } else if(action==='close-pi-preview'){$('#pi-preview')?.remove();
  } else if(action==='sales-message'){const m=await api(`/api/sales-quotes/${node.dataset.id}/${node.dataset.type}`);const text=m.message||`${m.subject}\n\n${m.body}`;await navigator.clipboard?.writeText(text);toast(`${node.dataset.type} message copied`);
