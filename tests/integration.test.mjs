...
test('health check and application shell are available', async () => {
  ...
  assert.equal(database.migrationVersion, '030_product_module_finalization');
...
test('Module 08A builds variants, configurable attributes, product relationships, and protected quote snapshots', async () => {
  ...
  const createAxis=async(product,category,name)=>{const response=await fetch(`http://127.0.0.1:${port}/api/product-attributes`,{method:'POST',headers:{Cookie:admin.cookie,'Content-Type':'application/json'},body:JSON.stringify({name_en:name,name_zh:name,data_type:'Text',category_configs:[{category_id:category.id,can_be_variant_axis:true,show_on_product:true}]})});assert.equal(response.status,201);const attribute=(await response.json()).attribute;const configured=await fetch(`http://127.0.0.1:${port}/api/products/${product.id}/foundation`,{method:'PUT',headers:{Cookie:admin.cookie,'Content-Type':'application/json'},body:JSON.stringify({variant_axis_attribute_ids:[attribute.id]})});assert.equal(configured.status,200);return attribute.id};
  const productAxes=new Map([[atlas.id,await createAxis(atlas,tableTopCategory,'M08 Table Top Size')],[harbor.id,await createAxis(harbor,chairCategory,'M08 Chair Format')]]);
  const addVariant=async(product,body)=>{const axisId=productAxes.get(product.id);const response=await fetch(`http://127.0.0.1:${port}/api/products/${product.id}/variants`,{method:'POST',headers:{Cookie:admin.cookie,'Content-Type':'application/json'},body:JSON.stringify({...body,option_values:[{attribute_id:axisId,value:body.variant_name}]})});assert.equal(response.status,201);return (await response.json()).variant};
  ...
  productAxes.set(stainlessBase.id,await createAxis(stainlessBase,tableBaseCategory,'M08 Table Base Size'));
  ...
  productAxes.set(dishwasher.id,await createAxis(dishwasher,kitchenCategory,'M08 Dishwasher Voltage'));
...
test('Product Foundation Correction enforces bilingual attributes, category contracts, variant axes, inheritance, and role-safe writes',async()=>{
  ...
  const rejectedAxes=await fetch(`http://127.0.0.1:${port}/api/products/${created.product.id}/foundation`,{method:'PUT',headers:headers(designer),body:JSON.stringify({variant_axis_attribute_ids:definitions.map(attribute=>attribute.id)})});assert.equal(rejectedAxes.status,409);
  const foundation=await fetch(`http://127.0.0.1:${port}/api/products/${created.product.id}/foundation`,{method:'PUT',headers:headers(designer),body:JSON.stringify({attribute_values:[{attribute_id:definitions[0].id,value:'Black'}],variant_axis_attribute_ids:[definitions[0].id]})});assert.equal(foundation.status,200);
  const variantResponse=await fetch(`http://127.0.0.1:${port}/api/products/${created.product.id}/variants`,{method:'POST',headers:headers(designer),body:JSON.stringify({variant_name:'Black',variant_sku:`PFC-V-${Date.now()}`,supplier_cost:888,option_values:[{attribute_id:definitions[0].id,value:'Black'}]})}),variant=await variantResponse.json();assert.equal(variantResponse.status,201,variant.error);assert.equal('supplier_cost' in variant.variant,false);
  const detail=await fetch(`http://127.0.0.1:${port}/api/products/${created.product.id}`,{headers:{Cookie:admin.cookie}}).then(response=>response.json());assert.equal(detail.foundation.variantAxes.length,1);assert.equal(detail.foundation.variantAxisWarning,null);assert.equal(detail.foundation.variants[0].option_values.length,1);assert.equal(detail.foundation.variants[0].effective_values.material,'Steel');assert.equal(detail.foundation.variants[0].supplier_cost,null);
...
test('V5.3 PIC-1 exposes Product Intelligence API boundary without leaking supplier data',async()=>{
  ...
  const axisResponse=await fetch(`http://127.0.0.1:${port}/api/product-attributes`,{method:'POST',headers:{Cookie:owner.cookie,'Content-Type':'application/json'},body:JSON.stringify({name_en:'PIC1 Format',name_zh:'PIC1 Format',data_type:'Text',category_configs:[{category_id:category.id,can_be_variant_axis:true,show_on_product:true}]})}),axis=(await axisResponse.json()).attribute;assert.equal(axisResponse.status,201);
  const axisSave=await fetch(`http://127.0.0.1:${port}/api/products/${created.product.id}/foundation`,{method:'PUT',headers:{Cookie:owner.cookie,'Content-Type':'application/json'},body:JSON.stringify({variant_axis_attribute_ids:[axis.id]})});assert.equal(axisSave.status,200);
  const variantResponse=await fetch(`http://127.0.0.1:${port}/api/products/${created.product.id}/variants`,{method:'POST',headers:{Cookie:owner.cookie,'Content-Type':'application/json'},body:JSON.stringify({variant_name:'Standard',variant_sku:'PIC1-STD',dimensions:'520 x 560 x 820 mm',material:'Ash Wood',finish:'Matte PU',color:'Walnut',reference_price:128,cost_price:42,supplier_cost:42,supplier_sku:'PIC1-SUP-V1',supplier_currency:'USD',status:'Active',option_values:[{attribute_id:axis.id,value:'Standard'}]})});
