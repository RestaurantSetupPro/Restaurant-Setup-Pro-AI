...
export function createSearchExecutionService({ db, audit }) {
  ...
  const readExecution = id => {
    ...
    if (!row) return null;
    const terminalPhase=row.status==='Completed'?'Complete':row.status==='Partially Completed'?'Partial Complete':['Failed','Cancelled','Paused'].includes(row.status)?row.status:null;
    return { ...row, phase:terminalPhase||row.phase, request_snapshot_json: json(row.request_snapshot_json), limits_json: json(row.limits_json), estimate_json: json(row.estimate_json), checkpoint_json: json(row.checkpoint_json) };
  };
