...
function initializeDatabase() {
  ...
  try {
    ...
    if (databaseUrl) {
      ...
      ensureProductFoundationCorrection();
      ensureProductModuleFinalization();
      ensureMediaColumns();
...
const productTagTypes = Object.freeze(['Use Case','Feature','Style','Sales','Store Type Tags','Style Tags','Business Tags']);
...
const productSalesModes = Object.freeze(['standard_sale','quote_only','customizable','project_only']);
const productPublishStatuses = Object.freeze(['draft','ready','published','archived']);
const priceDisplayModes = Object.freeze(['exact_price','starting_from','request_quote','hidden']);
const productRelationshipTypes = Object.freeze(['complementary','alternative','frequently_bought_together','upgrade']);
...
function ensureProductFoundationCorrection() {
  const ensure = (table, columns) => {
    const existing = new Set(db.prepare(`PRAGMA table_info(${table})`).all().map(column => column.name));
    for (const [name, type] of Object.entries(columns)) if (!existing.has(name)) db.exec(`ALTER TABLE ${table} ADD COLUMN ${name} ${type}`);
  };
  ensure('product_attribute_definitions', { name_en:'TEXT', name_zh:'TEXT', minimum_value:'REAL', maximum_value:'REAL' });
  ensure('product_attribute_category_links', {
    required:'INTEGER NOT NULL DEFAULT 0', filterable:'INTEGER NOT NULL DEFAULT 0', show_on_product:'INTEGER NOT NULL DEFAULT 1',
    show_on_quote:'INTEGER NOT NULL DEFAULT 0', show_on_pi:'INTEGER NOT NULL DEFAULT 0', internal_only:'INTEGER NOT NULL DEFAULT 0',
    can_be_variant_axis:'INTEGER NOT NULL DEFAULT 0', sort_order:'INTEGER NOT NULL DEFAULT 0', default_unit:'TEXT'
  });
  db.prepare("UPDATE product_attribute_definitions SET name_en=name WHERE name_en IS NULL OR TRIM(name_en)='' ").run();
  db.prepare("UPDATE product_attribute_definitions SET name_zh=name WHERE name_zh IS NULL OR TRIM(name_zh)='' ").run();
  db.exec(`CREATE TABLE IF NOT EXISTS product_variant_axes (
    product_id INTEGER NOT NULL REFERENCES products(id) ON DELETE CASCADE,
    attribute_id INTEGER NOT NULL REFERENCES product_attribute_definitions(id) ON DELETE RESTRICT,
    sort_order INTEGER NOT NULL DEFAULT 0, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY(product_id,attribute_id))`);
  db.exec(`CREATE TABLE IF NOT EXISTS product_variant_option_values (
    variant_id INTEGER NOT NULL REFERENCES product_variants(id) ON DELETE CASCADE,
    attribute_id INTEGER NOT NULL REFERENCES product_attribute_definitions(id) ON DELETE RESTRICT,
    value TEXT NOT NULL, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY(variant_id,attribute_id))`);
  db.exec('CREATE INDEX IF NOT EXISTS idx_product_variant_axes_product ON product_variant_axes(product_id,sort_order,attribute_id)');
  db.exec('CREATE INDEX IF NOT EXISTS idx_product_variant_option_values_variant ON product_variant_option_values(variant_id,attribute_id)');
}

function ensureProductModuleFinalization() {
  const ensure = (table, columns) => {
    const existing = new Set(db.prepare(`PRAGMA table_info(${table})`).all().map(column => column.name));
    for (const [name, type] of Object.entries(columns)) if (!existing.has(name)) db.exec(`ALTER TABLE ${table} ADD COLUMN ${name} ${type}`);
  };
  ensure('product_categories', { name_en:'TEXT', name_zh:'TEXT', category_code:'TEXT', description_en:'TEXT', description_zh:'TEXT' });
  ensure('product_attribute_category_links', { searchable:'INTEGER NOT NULL DEFAULT 0', show_on_storefront:'INTEGER NOT NULL DEFAULT 0' });
  ensure('product_variant_axes', { active:'INTEGER NOT NULL DEFAULT 1' });
  ensure('products', {
    sales_mode:"TEXT NOT NULL DEFAULT 'quote_only'", publish_status:"TEXT NOT NULL DEFAULT 'draft'",
    storefront_title_zh:'TEXT', storefront_title_en:'TEXT', storefront_description_zh:'TEXT', storefront_description_en:'TEXT',
    price_display_mode:"TEXT NOT NULL DEFAULT 'request_quote'", minimum_order_quantity:'REAL', lead_time_text:'TEXT', shipping_mode:'TEXT',
    request_quote_enabled:'INTEGER NOT NULL DEFAULT 1', customization_available:'INTEGER NOT NULL DEFAULT 0', variant_axis_review_status:"TEXT NOT NULL DEFAULT 'clear'"
  });
  db.prepare("UPDATE product_categories SET name_en=name WHERE name_en IS NULL OR TRIM(name_en)='' ").run();
  db.prepare("UPDATE product_categories SET name_zh=name WHERE name_zh IS NULL OR TRIM(name_zh)='' ").run();
  db.prepare("UPDATE product_categories SET category_code=UPPER(REPLACE(slug,'-','_')) WHERE category_code IS NULL OR TRIM(category_code)='' ").run();
  db.exec('CREATE UNIQUE INDEX IF NOT EXISTS uq_product_categories_code ON product_categories(category_code COLLATE NOCASE)');
  const duplicateAxes=db.prepare('SELECT product_id,COUNT(*) AS count FROM product_variant_axes WHERE active=1 GROUP BY product_id HAVING COUNT(*)>1').all();
  for(const row of duplicateAxes)db.prepare("UPDATE products SET variant_axis_review_status='needs_review' WHERE id=?").run(row.product_id);
  if(!duplicateAxes.length)db.exec('CREATE UNIQUE INDEX IF NOT EXISTS uq_product_variant_axes_one_active ON product_variant_axes(product_id) WHERE active=1');
  db.exec(`CREATE TABLE IF NOT EXISTS product_attribute_aliases (
    ...
    alias TEXT NOT NULL, normalized_alias TEXT NOT NULL, created_by INTEGER REFERENCES users(id) ON DELETE SET NULL,
    created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, UNIQUE(attribute_id,normalized_alias))`);
  db.exec('CREATE INDEX IF NOT EXISTS idx_product_attribute_aliases_lookup ON product_attribute_aliases(normalized_alias,attribute_id)');
  db.exec(`CREATE TABLE IF NOT EXISTS supplier_import_mapping_profiles (
    id INTEGER PRIMARY KEY AUTOINCREMENT, supplier_name TEXT NOT NULL, source_column TEXT NOT NULL, target_field TEXT NOT NULL,
    attribute_id INTEGER REFERENCES product_attribute_definitions(id) ON DELETE SET NULL, default_category_id INTEGER REFERENCES product_categories(id) ON DELETE SET NULL,
    currency TEXT, exchange_rate_rule TEXT, active INTEGER NOT NULL DEFAULT 1, created_by INTEGER REFERENCES users(id) ON DELETE SET NULL,
    updated_by INTEGER REFERENCES users(id) ON DELETE SET NULL, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, UNIQUE(supplier_name,source_column))`);
  db.exec('CREATE INDEX IF NOT EXISTS idx_supplier_import_mapping_profiles_supplier ON supplier_import_mapping_profiles(supplier_name COLLATE NOCASE,active)');
  db.exec(`CREATE TABLE IF NOT EXISTS channel_product_mappings (
    id INTEGER PRIMARY KEY AUTOINCREMENT, product_id INTEGER NOT NULL REFERENCES products(id) ON DELETE CASCADE,
    variant_id INTEGER REFERENCES product_variants(id) ON DELETE CASCADE, channel TEXT NOT NULL,
    external_product_id TEXT, external_variant_id TEXT, external_url TEXT, sync_status TEXT NOT NULL DEFAULT 'not_connected',
    last_synced_at TEXT, sync_error TEXT, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UNIQUE(product_id,variant_id,channel))`);
  db.exec('CREATE INDEX IF NOT EXISTS idx_channel_product_mappings_product ON channel_product_mappings(product_id,channel,sync_status)');
}
...
function canManageProductLibrary(user) {
  return Boolean(user && (['Admin', 'Owner', 'VA'].includes(user.role) || String(user.email).toLowerCase() === 'salesadmin@rspro.ai'));
}
...
function productInputForRole(body,user){const safe={...body};if(!canViewSensitiveProductData(user))for(const field of sensitiveProductFields)safe[field]=undefined;return safe}
...
function productWithTags(id) {
  ...
  product.main_image_url = usableMediaUrl(db.prepare("SELECT ma.file_url FROM product_media_links pml JOIN media_assets ma ON ma.id=pml.media_id WHERE pml.product_id=? AND ma.image_status!='Rejected' ORDER BY pml.is_primary DESC,CASE WHEN ma.image_type='Main Image' THEN 0 ELSE 1 END,pml.sort_order LIMIT 1").get(id)?.file_url)||null;
...
function productFoundation(id) {
  const product=db.prepare('SELECT * FROM products WHERE id=?').get(id),categoryId=product?.category_id;
  const definitions=attributeMasterRows().filter(attribute=>attribute.category_ids.includes(Number(categoryId))).map(attribute=>({...attribute,category_config:attribute.category_configs.find(config=>config.category_id===Number(categoryId))})).sort((left,right)=>Number(left.category_config?.sort_order||0)-Number(right.category_config?.sort_order||0)||left.name.localeCompare(right.name));
  const variantAxes=db.prepare(`SELECT pva.attribute_id,pva.sort_order,pad.name,pad.name_en,pad.name_zh,pad.code,pad.data_type,pad.unit
    FROM product_variant_axes pva JOIN product_attribute_definitions pad ON pad.id=pva.attribute_id WHERE pva.product_id=? AND pva.active=1 ORDER BY pva.sort_order,pad.name`).all(id);
  const variantRows=db.prepare(`SELECT pv.*,pib.source_file_name AS import_batch_file,iu.name AS imported_by_name,uu.name AS last_updated_by_name
      ...
      WHERE pv.product_id=? ORDER BY pv.sort_order,pv.id`).all(id).map(variant=>{
  ...
  return {
    ...
    attributeValues: db.prepare(`SELECT pav.*,pad.name,pad.name_en,pad.name_zh,pad.code,pad.unit,pv.variant_name,
      CASE WHEN EXISTS(SELECT 1 FROM product_attribute_category_links pacl WHERE pacl.attribute_id=pav.attribute_id AND pacl.category_id=?) THEN 1 ELSE 0 END AS applicable
      FROM product_attribute_values pav JOIN product_attribute_definitions pad ON pad.id=pav.attribute_id LEFT JOIN product_variants pv ON pv.id=pav.variant_id
      WHERE pav.product_id=? ORDER BY applicable DESC,pad.sort_order,pad.name,pv.sort_order`).all(categoryId,id),
    relatedProducts: db.prepare("SELECT p.id,p.sku,p.name FROM product_foundation_relationships pfr JOIN products p ON p.id=pfr.target_product_id WHERE pfr.source_product_id=? AND pfr.relationship_type IN ('related','complementary') ORDER BY pfr.sort_order,p.name").all(id),
...
function productIntelligenceListItem(product, user) {
  ...
  return redactSensitiveProductData({
    ...
    channel_readiness:{ready:missing.length===0,missing_fields:missing},
...
function productIntelligenceDetailData(id, user) {
  ...
  const payload = {
    basic_information: {
      ...
      updated_at: product.updated_at
      ,sales_mode:product.sales_mode,publish_status:product.publish_status,price_display_mode:product.price_display_mode,
      storefront_title_zh:product.storefront_title_zh,storefront_title_en:product.storefront_title_en,
      storefront_description_zh:product.storefront_description_zh,storefront_description_en:product.storefront_description_en,
      request_quote_enabled:Boolean(product.request_quote_enabled),customization_available:Boolean(product.customization_available)
    },
    attributes: {
    ...
    relationships: {
      ...
      unified: foundation.relationships
    },
    channel_readiness:{publish_status:product.publish_status,sales_mode:product.sales_mode,price_display_mode:product.price_display_mode,mappings:foundation.channelMappings,external_api_called:false}
  };
...
const productAttributeTypes = Object.freeze(['Text','Number','Select','Multi-select','Boolean','Color','Dimension','Date']);
function attributeCodeFromEnglishName(value){return String(value||'').normalize('NFKD').replace(/[\u0300-\u036f]/g,'').toLowerCase().trim().replace(/[^a-z0-9]+/g,'_').replace(/^_+|_+$/g,'').slice(0,80)}
function validateAttributeCode(value){const code=String(value||'').trim();if(!/^[a-z][a-z0-9]*(?:_[a-z0-9]+)*$/.test(code)){const error=new Error('Attribute code must use lowercase English letters, numbers, and underscores.');error.status=400;throw error}return code}
function attributeCategoryConfigs(attributeId){return db.prepare(`SELECT category_id,required,searchable,filterable,show_on_product,show_on_quote,show_on_pi,show_on_storefront,internal_only,can_be_variant_axis,sort_order,default_unit
  FROM product_attribute_category_links WHERE attribute_id=? ORDER BY sort_order,category_id`).all(attributeId).map(row=>({...row,category_id:Number(row.category_id)}))}
function normalizedAttributeAlias(value){return String(value||'').normalize('NFKC').toLowerCase().replace(/[\s_\-.,/\\:;()\[\]{}]+/g,'')}
function matchProductAttribute(sourceColumn,categoryId=null){const normalized=normalizedAttributeAlias(sourceColumn),rows=attributeMasterRows().filter(attribute=>!categoryId||attribute.category_ids.includes(Number(categoryId)));for(const priority of ['code','name_en','name_zh','aliases']){const match=rows.find(attribute=>priority==='aliases'?attribute.aliases.some(alias=>normalizedAttributeAlias(alias.alias)===normalized):normalizedAttributeAlias(attribute[priority])===normalized);if(match)return {attribute:match,match_type:priority,confidence:priority==='code'?100:priority==='aliases'?90:95}}return null}
function attributeMasterRows(){return db.prepare('SELECT * FROM product_attribute_definitions ORDER BY sort_order,name').all().map(attribute=>{const category_configs=attributeCategoryConfigs(attribute.id);return {...attribute,name_en:attribute.name_en||attribute.name,name_zh:attribute.name_zh||attribute.name,category_ids:category_configs.map(row=>row.category_id),category_configs,used_count:Number(db.prepare('SELECT COUNT(*) AS count FROM product_attribute_values WHERE attribute_id=?').get(attribute.id).count)+Number(db.prepare('SELECT COUNT(*) AS count FROM product_variant_axes WHERE attribute_id=? AND active=1').get(attribute.id).count),aliases:db.prepare('SELECT id,alias FROM product_attribute_aliases WHERE attribute_id=? ORDER BY alias').all(attribute.id),options:db.prepare('SELECT * FROM product_attribute_options WHERE attribute_id=? ORDER BY sort_order,option_value').all(attribute.id)}})}
function syncAttributeMaster(attributeId,body){
  if(Array.isArray(body.category_configs)||Array.isArray(body.category_ids)){
    const configs=Array.isArray(body.category_configs)?body.category_configs:normalizedIds(body.category_ids).map((category_id,index)=>({category_id,sort_order:index}));
    db.prepare('DELETE FROM product_attribute_category_links WHERE attribute_id=?').run(attributeId);
    const insert=db.prepare(`INSERT INTO product_attribute_category_links(attribute_id,category_id,required,searchable,filterable,show_on_product,show_on_quote,show_on_pi,show_on_storefront,internal_only,can_be_variant_axis,sort_order,default_unit)
      VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)`);
    configs.filter(config=>Number(config.category_id)).forEach((config,index)=>insert.run(attributeId,Number(config.category_id),config.required?1:0,config.searchable?1:0,config.filterable?1:0,config.show_on_product===false?0:1,config.show_on_quote?1:0,config.show_on_pi?1:0,config.show_on_storefront?1:0,config.internal_only?1:0,config.can_be_variant_axis?1:0,Number(config.sort_order??index),String(config.default_unit||'').trim()||null));
  }
  if(Array.isArray(body.aliases)||typeof body.aliases==='string'){
    const aliases=Array.isArray(body.aliases)?body.aliases:String(body.aliases).split(/\r?\n|,/);
    const existing=new Set(db.prepare('SELECT normalized_alias FROM product_attribute_aliases WHERE attribute_id=?').all(attributeId).map(row=>row.normalized_alias));
    const insertAlias=db.prepare('INSERT OR IGNORE INTO product_attribute_aliases(attribute_id,alias,normalized_alias,created_by) VALUES(?,?,?,?)');
    for(const value of aliases){const alias=String(typeof value==='string'?value:value.alias||'').trim(),normalized=normalizedAttributeAlias(alias);if(alias&&normalized&&!existing.has(normalized)){insertAlias.run(attributeId,alias,normalized,body.alias_created_by||null);existing.add(normalized)}}
  }
  if(Array.isArray(body.options)){db.prepare('DELETE FROM product_attribute_options WHERE attribute_id=?').run(attributeId);const insert=db.prepare('INSERT INTO product_attribute_options(attribute_id,option_value,active,sort_order) VALUES(?,?,?,?)');body.options.map(option=>typeof option==='string'?{option_value:option}:option).filter(option=>String(option.option_value||'').trim()).forEach((option,index)=>insert.run(attributeId,String(option.option_value).trim(),option.active===false?0:1,Number(option.sort_order??index)))}
}
...
function variantFieldValues(input, existing = {}, user = null) {
  const body=user?productInputForRole(input,user):input;
...
function syncVariantOptionValues(productId,variantId,optionValues){
  if(!Array.isArray(optionValues))return;
  const axes=new Set(db.prepare('SELECT attribute_id FROM product_variant_axes WHERE product_id=?').all(productId).map(row=>Number(row.attribute_id)));
  db.prepare('DELETE FROM product_variant_option_values WHERE variant_id=?').run(variantId);
  const insert=db.prepare('INSERT INTO product_variant_option_values(variant_id,attribute_id,value) VALUES(?,?,?)');
  for(const option of optionValues){const attributeId=Number(option.attribute_id);if(!axes.has(attributeId)){const error=new Error('Variant option must use an enabled Product Variant Axis.');error.status=400;throw error}const value=String(option.value||'').trim();if(value)insert.run(variantId,attributeId,value)}
}
...
function approveProductImportDraft(draftId,user,action='create_new'){
  ...
  if(draft.mapped_product?.auto_combination_blocked&&!String(action).startsWith('variant_axis:')&&action!=='split')throw Object.assign(new Error('Two changing fields were detected. Select one Variant Option or split this draft into separate Products before approval.'),{status:409});
  if(draft.mapped_product?.auto_combination_blocked&&action==='split')throw Object.assign(new Error('Use the Split action to create separate review drafts before approval.'),{status:409});
...
const handlers = {
  ...
  async mutateProduct(req, res, id = null) {
    const user = currentUser(req);
    if (!canManageProductLibrary(user)&&user?.role!=='Designer') return json(res, user ? 403 : 401, { error: 'Product content editing access is required.' });
    ...
    const salesMode=String(body.sales_mode??existing?.sales_mode??'quote_only'),publishStatus=String(body.publish_status??existing?.publish_status??'draft'),priceDisplayMode=String(body.price_display_mode??existing?.price_display_mode??'request_quote');
    if(!productSalesModes.includes(salesMode))return json(res,400,{error:'Sales mode is not supported.'});
    if(!productPublishStatuses.includes(publishStatus))return json(res,400,{error:'Publish status is not supported.'});
    if(!priceDisplayModes.includes(priceDisplayMode))return json(res,400,{error:'Price display mode is not supported.'});
    ...
    try {
      ...
      db.prepare(`UPDATE products SET sales_mode=?,publish_status=?,storefront_title_zh=?,storefront_title_en=?,storefront_description_zh=?,storefront_description_en=?,price_display_mode=?,minimum_order_quantity=?,lead_time_text=?,shipping_mode=?,request_quote_enabled=?,customization_available=? WHERE id=?`).run(
        salesMode,publishStatus,String(body.storefront_title_zh??existing?.storefront_title_zh??'').trim()||null,String(body.storefront_title_en??existing?.storefront_title_en??'').trim()||null,
        String(body.storefront_description_zh??existing?.storefront_description_zh??'').trim()||null,String(body.storefront_description_en??existing?.storefront_description_en??'').trim()||null,priceDisplayMode,
        body.minimum_order_quantity===''||body.minimum_order_quantity==null?(existing?.minimum_order_quantity??null):Number(body.minimum_order_quantity),String(body.lead_time_text??existing?.lead_time_text??'').trim()||null,
        String(body.shipping_mode??existing?.shipping_mode??'').trim()||null,body.request_quote_enabled===undefined?(existing?.request_quote_enabled??1):body.request_quote_enabled?1:0,
        body.customization_available===undefined?(existing?.customization_available??0):body.customization_available?1:0,id);
  ...
  async generateAiContent(req, res, id) {
    ...
    const product = productKnowledge(id);
    if (!product) return json(res, 404, { error: 'Product not found.' });
    const body = productInputForRole(await readJson(req),user);
  ...
  productCategories(req,res){const user=currentUser(req);if(!requires(user,'products'))return json(res,user?403:401,{error:'Access denied.'});const categories=db.prepare('SELECT pc.*,COUNT(p.id) AS product_count FROM product_categories pc LEFT JOIN products p ON p.category_id=pc.id GROUP BY pc.id ORDER BY pc.sort_order,pc.name').all().map(category=>({...category,attribute_count:Number(db.prepare('SELECT COUNT(*) AS count FROM product_attribute_category_links WHERE category_id=?').get(category.id).count),attributes:db.prepare(`SELECT pad.id,pad.name,pad.name_en,pad.name_zh,pad.code,pacl.required,pacl.searchable,pacl.filterable,pacl.show_on_product,pacl.show_on_quote,pacl.show_on_pi,pacl.show_on_storefront,pacl.internal_only,pacl.can_be_variant_axis,pacl.sort_order,pacl.default_unit FROM product_attribute_category_links pacl JOIN product_attribute_definitions pad ON pad.id=pacl.attribute_id WHERE pacl.category_id=? ORDER BY pacl.sort_order,pad.name`).all(category.id)}));return json(res,200,{categories,attribute_options:attributeMasterRows().filter(attribute=>Number(attribute.active)===1)});},
  async createProductCategory(req,res){const user=currentUser(req);if(!canManageProductLibrary(user))return json(res,user?403:401,{error:'Administrator access required.'});const b=await readJson(req);const nameEn=requiredText(b.name_en??b.name,'English category name'),nameZh=requiredText(b.name_zh??b.name,'Chinese category name'),slug=String(b.slug||makeCode(nameEn).toLowerCase()).trim(),code=String(b.category_code||makeCode(nameEn)).trim().toUpperCase();try{const row=db.prepare('INSERT INTO product_categories(name,name_en,name_zh,category_code,slug,description,description_en,description_zh,active,sort_order) VALUES(?,?,?,?,?,?,?,?,?,?) RETURNING *').get(nameEn,nameEn,nameZh,code,slug,String(b.description_en??b.description??'').trim()||null,String(b.description_en??b.description??'').trim()||null,String(b.description_zh||'').trim()||null,b.active===false?0:1,Number(b.sort_order||0));return json(res,201,{category:row})}catch(error){return json(res,409,{error:'Category name, code, or slug already exists.'})}},
  async updateProductCategory(req,res,id){const user=currentUser(req);if(!canManageProductLibrary(user))return json(res,user?403:401,{error:'Administrator access required.'});const existing=db.prepare('SELECT * FROM product_categories WHERE id=?').get(id);if(!existing)return json(res,404,{error:'Category not found.'});const b=await readJson(req),nameEn=requiredText(b.name_en??b.name??existing.name_en??existing.name,'English category name'),nameZh=requiredText(b.name_zh??existing.name_zh??existing.name,'Chinese category name');db.prepare('UPDATE product_categories SET name=?,name_en=?,name_zh=?,category_code=?,slug=?,description=?,description_en=?,description_zh=?,active=?,sort_order=?,updated_at=CURRENT_TIMESTAMP WHERE id=?').run(nameEn,nameEn,nameZh,String(b.category_code??existing.category_code??makeCode(nameEn)).trim().toUpperCase(),String(b.slug??existing.slug).trim(),String(b.description_en??b.description??existing.description_en??existing.description??'').trim()||null,String(b.description_en??b.description??existing.description_en??existing.description??'').trim()||null,String(b.description_zh??existing.description_zh??'').trim()||null,b.active===undefined?existing.active:b.active?1:0,Number(b.sort_order??existing.sort_order??0),id);return json(res,200,{category:db.prepare('SELECT * FROM product_categories WHERE id=?').get(id)})},
  async updateProductCategoryAttributes(req,res,id){const user=currentUser(req);if(!canManageProductLibrary(user))return json(res,user?403:401,{error:'Product master data access required.'});if(!db.prepare('SELECT id FROM product_categories WHERE id=?').get(id))return json(res,404,{error:'Category not found.'});const b=await readJson(req),configs=Array.isArray(b.attributes)?b.attributes:[];db.exec('BEGIN IMMEDIATE');try{db.prepare('DELETE FROM product_attribute_category_links WHERE category_id=?').run(id);const insert=db.prepare(`INSERT INTO product_attribute_category_links(attribute_id,category_id,required,searchable,filterable,show_on_product,show_on_quote,show_on_pi,show_on_storefront,internal_only,can_be_variant_axis,sort_order,default_unit) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)`);configs.filter(config=>Number(config.attribute_id)).forEach((config,index)=>insert.run(Number(config.attribute_id),id,config.required?1:0,config.searchable?1:0,config.filterable?1:0,config.show_on_product===false?0:1,config.show_on_quote?1:0,config.show_on_pi?1:0,config.show_on_storefront?1:0,config.internal_only?1:0,config.can_be_variant_axis?1:0,Number(config.sort_order??index),String(config.default_unit||'').trim()||null));db.exec('COMMIT')}catch(error){if(db.isTransaction)db.exec('ROLLBACK');throw error}audit(user.id,'configure_attributes','product_categories',String(id),{attributeCount:configs.length});return handlers.productCategories(req,res)},
  ...
  productVariants(req,res){const user=currentUser(req);if(!requires(user,'products'))return json(res,user?403:401,{error:'Access denied.'});const products=db.prepare("SELECT id,name,sku FROM products WHERE status!='archived' ORDER BY name").all().map(product=>({...product,variant_axes:productFoundation(product.id).variantAxes}));const variants=db.prepare('SELECT pv.*,p.name AS product_name,p.sku AS product_sku FROM product_variants pv JOIN products p ON p.id=pv.product_id ORDER BY p.name,pv.sort_order,pv.id').all().map(variant=>({...variant,option_values:db.prepare('SELECT attribute_id,value FROM product_variant_option_values WHERE variant_id=? ORDER BY attribute_id').all(variant.id)}));return json(res,200,{variants:redactSensitiveProductData(variants,user),products,capabilities:{canViewSensitive:canViewSensitiveProductData(user),canManage:canManageProductLibrary(user)}})},

async createProductVariant(req,res,productId){const user=currentUser(req);if(!['Admin','Owner','Designer','VA'].includes(user?.role))return json(res,user?403:401,{error:'Product editing is not allowed.'});if(!db.prepare('SELECT id FROM products WHERE id=?').get(productId))return json(res,404,{error:'Product not found.'});const axes=db.prepare('SELECT attribute_id FROM product_variant_axes WHERE product_id=? AND active=1').all(productId);if(axes.length!==1)return json(res,409,{error:'Set exactly one Variant Option before creating Product Variants. / 创建具体规格前必须设置且只能设置一个规格变化字段。'});const b=productInputForRole(await readJson(req),user),options=Array.isArray(b.option_values)?b.option_values.filter(option=>String(option.value||'').trim()):[];if(options.length!==1||Number(options[0].attribute_id)!==Number(axes[0].attribute_id))return json(res,409,{error:'Each Product Variant must have exactly one value for the active Variant Option.'});const status=String(b.status||'Active');if(!masterValues('Product Status Options',variantStatuses).includes(status))return json(res,400,{error:'Variant status is not supported.'});const values=variantFieldValues(b,{},user);values[0]=requiredText(values[0],'Variant name');db.exec('BEGIN IMMEDIATE');try{const row=db.prepare('INSERT INTO product_variants(product_id,variant_name,variant_sku,dimensions,material,finish,color,reference_price,cost_price,moq,lead_time_days,cbm,gross_weight_kg,net_weight_kg,packing_info,default_supplier,supplier_sku,supplier_cost,supplier_lead_time_days,supplier_moq,supplier_notes,status,sort_order,last_updated_by) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) RETURNING *').get(productId,...values,status,Number(b.sort_order||0),user.id);syncVariantOptionValues(productId,row.id,options);db.exec('COMMIT');return json(res,201,{variant:redactSensitiveProductData({...row,option_values:options},user)})}catch(error){if(db.isTransaction)db.exec('ROLLBACK');throw error}},
  async updateProductVariant(req,res,productId,variantId){const user=currentUser(req);if(!['Admin','Owner','Designer','VA'].includes(user?.role))return json(res,user?403:401,{error:'Product editing is not allowed.'});const existing=db.prepare('SELECT * FROM product_variants WHERE id=? AND product_id=?').get(variantId,productId);if(!existing)return json(res,404,{error:'Variant not found.'});const b=await readJson(req),status=String(b.status??existing.status);if(!masterValues('Product Status Options',variantStatuses).includes(status))return json(res,400,{error:'Variant status is not supported.'});const values=variantFieldValues(b,existing,user);values[0]=requiredText(values[0],'Variant name');db.exec('BEGIN IMMEDIATE');try{db.prepare('UPDATE product_variants SET variant_name=?,variant_sku=?,dimensions=?,material=?,finish=?,color=?,reference_price=?,cost_price=?,moq=?,lead_time_days=?,cbm=?,gross_weight_kg=?,net_weight_kg=?,packing_info=?,default_supplier=?,supplier_sku=?,supplier_cost=?,supplier_lead_time_days=?,supplier_moq=?,supplier_notes=?,status=?,sort_order=?,last_updated_by=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND product_id=?').run(...values,status,Number(b.sort_order??existing.sort_order),user.id,variantId,productId);syncVariantOptionValues(productId,variantId,b.option_values);db.exec('COMMIT');return json(res,200,{variant:redactSensitiveProductData(db.prepare('SELECT * FROM product_variants WHERE id=?').get(variantId),user)})}catch(error){if(db.isTransaction)db.exec('ROLLBACK');throw error}},
  ...
  productAttributes(req,res){const user=currentUser(req);if(!requires(user,'products'))return json(res,user?403:401,{error:'Access denied.'});return json(res,200,{attributes:attributeMasterRows(),types:productAttributeTypes})},
  matchProductImportField(req,res,url){const user=currentUser(req);if(!requires(user,'imports'))return json(res,user?403:401,{error:'Access denied.'});const source=String(url.searchParams.get('source')||''),categoryId=Number(url.searchParams.get('category_id')||0)||null,standardFields=['Product Name','Product Code','Category','Description','Image','Price','Currency','Supplier','Cost'];const normalized=normalizedAttributeAlias(source),standard=standardFields.find(field=>normalizedAttributeAlias(field)===normalized);if(standard)return json(res,200,{source_column:source,target_type:'standard',target_field:standard,confidence:100,requires_human_confirmation:false});const match=matchProductAttribute(source,categoryId);return json(res,200,match?{source_column:source,target_type:'attribute',target_field:match.attribute.code,attribute_id:match.attribute.id,match_type:match.match_type,confidence:match.confidence,requires_human_confirmation:false}:{source_column:source,target_type:null,target_field:null,confidence:0,requires_human_confirmation:true})},
  importMappingProfiles(req,res,url){const user=currentUser(req);if(!requires(user,'imports'))return json(res,user?403:401,{error:'Access denied.'});const supplier=String(url.searchParams.get('supplier')||'').trim(),where=supplier?'WHERE LOWER(supplier_name)=LOWER(?) AND active=1':'WHERE active=1',rows=db.prepare(`SELECT * FROM supplier_import_mapping_profiles ${where} ORDER BY supplier_name,source_column`).all(...(supplier?[supplier]:[]));return json(res,200,{profiles:rows,requires_human_review:true})},
  async saveImportMappingProfile(req,res){const user=currentUser(req);if(!canManageProductLibrary(user))return json(res,user?403:401,{error:'Product master data access required.'});const b=await readJson(req),supplier=requiredText(b.supplier_name,'Supplier'),source=requiredText(b.source_column,'Source column'),target=requiredText(b.target_field,'Target field'),attributeId=Number(b.attribute_id)||null,categoryId=Number(b.default_category_id)||null;db.prepare(`INSERT INTO supplier_import_mapping_profiles(supplier_name,source_column,target_field,attribute_id,default_category_id,currency,exchange_rate_rule,active,created_by,updated_by) VALUES(?,?,?,?,?,?,?,1,?,?) ON CONFLICT(supplier_name,source_column) DO UPDATE SET target_field=excluded.target_field,attribute_id=excluded.attribute_id,default_category_id=excluded.default_category_id,currency=excluded.currency,exchange_rate_rule=excluded.exchange_rate_rule,active=1,updated_by=excluded.updated_by,updated_at=CURRENT_TIMESTAMP`).run(supplier,source,target,attributeId,categoryId,String(b.currency||'').trim()||null,String(b.exchange_rate_rule||'').trim()||null,user.id,user.id);audit(user.id,'save_import_mapping_profile','supplier_import_mapping_profiles',`${supplier}:${source}`);return json(res,200,{profile:db.prepare('SELECT * FROM supplier_import_mapping_profiles WHERE supplier_name=? AND source_column=?').get(supplier,source),requires_human_review:true})},
  async addProductAttributeAlias(req,res,id){const user=currentUser(req);if(!canManageProductLibrary(user))return json(res,user?403:401,{error:'Product master data access required.'});if(!db.prepare('SELECT id FROM product_attribute_definitions WHERE id=?').get(id))return json(res,404,{error:'Attribute not found.'});const b=await readJson(req),alias=requiredText(b.alias,'Attribute alias'),normalized=normalizedAttributeAlias(alias);db.prepare('INSERT OR IGNORE INTO product_attribute_aliases(attribute_id,alias,normalized_alias,created_by) VALUES(?,?,?,?)').run(id,alias,normalized,user.id);audit(user.id,'confirm_import_alias','product_attribute_definitions',String(id),{alias});return json(res,201,{attribute:attributeMasterRows().find(attribute=>attribute.id===id)})},
  productTagRecommendations(req,res,id){const user=currentUser(req);if(!requires(user,'products'))return json(res,user?403:401,{error:'Access denied.'});const product=productWithTags(id);if(!product)return json(res,404,{error:'Product not found.'});const text=[product.name,product.summary,product.category,product.materials,product.recommended_usage].filter(Boolean).join(' ').toLowerCase(),assigned=new Set(product.tag_ids||[]),recommendations=db.prepare(`SELECT id,tag_name,tag_type,description FROM system_tags WHERE active=1 AND tag_type IN (${productTagTypes.map(()=>'?').join(',')}) ORDER BY sort_order,tag_name`).all(...productTagTypes).filter(tag=>!assigned.has(tag.id)&&String(tag.tag_name).toLowerCase().split(/\s+/).some(keyword=>keyword.length>2&&text.includes(keyword))).map(tag=>({...tag,reason:'Existing tag matched by category, name, description, or fixed attributes.'}));return json(res,200,{recommendations,requires_human_confirmation:true,created_tags:0,provider:'rules',external_ai_called:false})},
  async upsertChannelMapping(req,res,id){const user=currentUser(req);if(!['Admin','Owner'].includes(user?.role))return json(res,user?403:401,{error:'Business Administrator access required.'});if(!db.prepare('SELECT id FROM products WHERE id=?').get(id))return json(res,404,{error:'Product not found.'});const b=await readJson(req),channel=String(b.channel||'');if(!['shopify','woocommerce','alibaba','other'].includes(channel))return json(res,400,{error:'Channel is not supported.'});db.prepare(`INSERT INTO channel_product_mappings(product_id,variant_id,channel,external_product_id,external_variant_id,external_url,sync_status,updated_at) VALUES(?,?,?,?,?,?,?,CURRENT_TIMESTAMP)
      ON CONFLICT(product_id,variant_id,channel) DO UPDATE SET external_product_id=excluded.external_product_id,external_variant_id=excluded.external_variant_id,external_url=excluded.external_url,sync_status=excluded.sync_status,updated_at=CURRENT_TIMESTAMP`).run(id,b.variant_id?Number(b.variant_id):null,channel,String(b.external_product_id||'').trim()||null,String(b.external_variant_id||'').trim()||null,String(b.external_url||'').trim()||null,'not_connected');return json(res,200,{mappings:productFoundation(id).channelMappings,external_api_called:false})},
  async createProductAttribute(req,res){const user=currentUser(req);if(!canManageProductLibrary(user))return json(res,user?403:401,{error:'Product master data access required.'});const b=await readJson(req);b.alias_created_by=user.id;if(!Array.isArray(b.category_ids)&&b.category_id)b.category_ids=[b.category_id];const nameEn=requiredText(b.name_en??b.name,'English name'),nameZh=requiredText(b.name_zh??b.name,'Chinese name'),code=validateAttributeCode(attributeCodeFromEnglishName(nameEn)),type=String(b.data_type||'Text');if(!productAttributeTypes.includes(type))return json(res,400,{error:'Attribute data type is not supported.'});if(b.active!==false&&(!nameEn||!nameZh))return json(res,400,{error:'Chinese and English names are required before activation.'});try{const row=db.prepare('INSERT INTO product_attribute_definitions(category_id,name,name_en,name_zh,code,data_type,unit,minimum_value,maximum_value,active,sort_order,show_in_library,show_on_website,show_in_quote,show_in_pi,internal_only) VALUES(NULL,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) RETURNING *').get(nameEn,nameEn,nameZh,code,type,String(b.unit||'').trim()||null,b.minimum_value===''||b.minimum_value==null?null:Number(b.minimum_value),b.maximum_value===''||b.maximum_value==null?null:Number(b.maximum_value),b.active===false?0:1,Number(b.sort_order||0),b.show_in_library===false?0:1,b.show_on_website?1:0,b.show_in_quote?1:0,b.show_in_pi?1:0,b.internal_only?1:0);syncAttributeMaster(row.id,b);return json(res,201,{attribute:attributeMasterRows().find(attribute=>attribute.id===row.id)})}catch(error){if(error?.code==='23505'||String(error.message).includes('UNIQUE constraint failed'))return json(res,409,{error:'Attribute code already exists. Use another English name or ask an administrator to adjust the code.'});throw error}},
  async updateProductAttribute(req,res,id){const user=currentUser(req);if(!canManageProductLibrary(user))return json(res,user?403:401,{error:'Product master data access required.'});const existing=db.prepare('SELECT * FROM product_attribute_definitions WHERE id=?').get(id);if(!existing)return json(res,404,{error:'Attribute not found.'});const b=await readJson(req);b.alias_created_by=user.id;const nameEn=requiredText(b.name_en??b.name??existing.name_en??existing.name,'English name'),nameZh=requiredText(b.name_zh??existing.name_zh??existing.name,'Chinese name'),used=Number(db.prepare('SELECT COUNT(*) AS count FROM product_attribute_values WHERE attribute_id=?').get(id).count)+Number(db.prepare('SELECT COUNT(*) AS count FROM product_variant_axes WHERE attribute_id=? AND active=1').get(id).count),generated=validateAttributeCode(attributeCodeFromEnglishName(nameEn)),requested=String(b.code||'').trim()||generated;let code=existing.code;if(!used)code=generated;else if(requested!==existing.code&&user.role!=='Admin')return json(res,409,{error:'Attribute code is locked because the attribute is already in use.'});else if(requested!==existing.code&&user.role==='Admin')code=validateAttributeCode(requested);const type=String(b.data_type??existing.data_type);if(!productAttributeTypes.includes(type))return json(res,400,{error:'Attribute data type is not supported.'});const active=b.active===undefined?existing.active:b.active?1:0;if(active&&(!nameEn||!nameZh))return json(res,400,{error:'Chinese and English names are required before activation.'});try{db.prepare('UPDATE product_attribute_definitions SET name=?,name_en=?,name_zh=?,code=?,data_type=?,unit=?,minimum_value=?,maximum_value=?,active=?,sort_order=?,show_in_library=?,show_on_website=?,show_in_quote=?,show_in_pi=?,internal_only=?,updated_at=CURRENT_TIMESTAMP WHERE id=?').run(nameEn,nameEn,nameZh,code,type,String(b.unit??existing.unit??'').trim()||null,b.minimum_value===''?null:b.minimum_value??existing.minimum_value,b.maximum_value===''?null:b.maximum_value??existing.maximum_value,active,Number(b.sort_order??existing.sort_order),b.show_in_library===undefined?existing.show_in_library:b.show_in_library?1:0,b.show_on_website===undefined?existing.show_on_website:b.show_on_website?1:0,b.show_in_quote===undefined?existing.show_in_quote:b.show_in_quote?1:0,b.show_in_pi===undefined?existing.show_in_pi:b.show_in_pi?1:0,b.internal_only===undefined?existing.internal_only:b.internal_only?1:0,id);syncAttributeMaster(id,b);return json(res,200,{attribute:attributeMasterRows().find(attribute=>attribute.id===id)})}catch(error){if(error?.code==='23505'||String(error.message).includes('UNIQUE constraint failed'))return json(res,409,{error:'Attribute code already exists. Use another English name or ask an administrator to adjust the code.'});throw error}},
  ...
  async updateProductFoundation(req,res,id){const user=currentUser(req);if(!['Admin','Owner','Designer','VA'].includes(user?.role))return json(res,user?403:401,{error:'Product editing is not allowed.'});const product=db.prepare('SELECT id,category_id FROM products WHERE id=?').get(id);if(!product)return json(res,404,{error:'Product not found.'});const b=await readJson(req);db.exec('BEGIN IMMEDIATE');try{
    if(Array.isArray(b.attribute_values)){const linked=new Set(db.prepare('SELECT attribute_id FROM product_attribute_category_links WHERE category_id=? AND show_on_product=1').all(product.category_id).map(row=>Number(row.attribute_id))),insertValue=db.prepare('INSERT INTO product_attribute_values(product_id,variant_id,attribute_id,value) VALUES(?,?,?,?)');for(const value of b.attribute_values){const attributeId=Number(value.attribute_id),variantId=value.variant_id?Number(value.variant_id):null;if(!linked.has(attributeId)){const error=new Error('Attribute is not configured for this product category.');error.status=400;throw error}if(variantId)db.prepare('DELETE FROM product_attribute_values WHERE product_id=? AND variant_id=? AND attribute_id=?').run(id,variantId,attributeId);else db.prepare('DELETE FROM product_attribute_values WHERE product_id=? AND variant_id IS NULL AND attribute_id=?').run(id,attributeId);const text=String(value.value||'').trim();if(text)insertValue.run(id,variantId,attributeId,text)}}
    if(Array.isArray(b.variant_axis_attribute_ids)){const allowedAxes=new Set(db.prepare('SELECT attribute_id FROM product_attribute_category_links WHERE category_id=? AND can_be_variant_axis=1').all(product.category_id).map(row=>Number(row.attribute_id))),axisIds=normalizedIds(b.variant_axis_attribute_ids);if(axisIds.length>1){const error=new Error('A Product can have only one active Variant Option. / 每个产品最多只能设置一个规格变化字段。');error.status=409;throw error}for(const attributeId of axisIds)if(!allowedAxes.has(attributeId)){const error=new Error('Variant Option must be enabled for the selected product category.');error.status=400;throw error}db.prepare('DELETE FROM product_variant_axes WHERE product_id=?').run(id);const insertAxis=db.prepare('INSERT INTO product_variant_axes(product_id,attribute_id,sort_order,active) VALUES(?,?,?,1)');axisIds.forEach((attributeId,index)=>insertAxis.run(id,attributeId,index));db.prepare("UPDATE products SET variant_axis_review_status='clear' WHERE id=?").run(id)}
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    const productCategoryAttributesMatch=url.pathname.match(/^\/api\/product-categories\/(\d+)\/attributes$/);
    if(productCategoryAttributesMatch&&req.method==='PUT')return await handlers.updateProductCategoryAttributes(req,res,Number(productCategoryAttributesMatch[1]));
    ...
    if(req.method==='GET'&&url.pathname==='/api/imports/field-match')return handlers.matchProductImportField(req,res,url);
    if(req.method==='GET'&&url.pathname==='/api/imports/mapping-profiles')return handlers.importMappingProfiles(req,res,url);
    if(req.method==='PUT'&&url.pathname==='/api/imports/mapping-profiles')return await handlers.saveImportMappingProfile(req,res);
    ...
    const productAttributeAliasMatch=url.pathname.match(/^\/api\/product-attributes\/(\d+)\/aliases$/);
    if(productAttributeAliasMatch&&req.method==='POST')return await handlers.addProductAttributeAlias(req,res,Number(productAttributeAliasMatch[1]));
    ...
    const productTagRecommendationMatch=url.pathname.match(/^\/api\/products\/(\d+)\/tag-recommendations$/);
    if(productTagRecommendationMatch&&req.method==='GET')return handlers.productTagRecommendations(req,res,Number(productTagRecommendationMatch[1]));
    const productChannelMappingMatch=url.pathname.match(/^\/api\/products\/(\d+)\/channel-mappings$/);
    if(productChannelMappingMatch&&req.method==='PUT')return await handlers.upsertChannelMapping(req,res,Number(productChannelMappingMatch[1]));
