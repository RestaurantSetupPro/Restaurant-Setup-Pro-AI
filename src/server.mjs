  ...
  try {
    ...
    if (databaseUrl) {
      ...
      if (process.env.RUN_MIGRATIONS !== 'false') {
        for (const migrationFile of ['001_initial_schema.sql', '002_product_intelligence.sql', '003_ai_product_content_factory.sql', '004_real_ai_image_generation.sql', '005_opportunity_intelligence_engine.sql', '006_ai_cost_control.sql', '007_sales_intelligence_part1.sql', '008_quote_pi_builder.sql', '009_custom_quote_items.sql', '010_global_pi_template.sql', '011_professional_pi_optimization.sql', '012_product_foundation.sql', '013_product_master_data.sql', '014_pim_foundation_upgrade.sql', '015_ai_product_import.sql', '016_product_library_business_readiness.sql', '017_product_price_engine.sql', '018_v53_ai_business_brain_foundation.sql', '019_v53_phase2a_customer_intelligence_mvp.sql', '020_v53_opportunity_discovery_assistant.sql', '021_v53_search_task_execution_center.sql', '022_v53_customer_intelligence_update_history.sql', '023_v53_search_result_storage.sql', '024_v53_customer_source.sql', '025_v53_ai_knowledge_center_foundation.sql', '026_v53_search_strategy_human_approval.sql', '027_v53_search_execution_foundation.sql', '028_search_execution_terminal_phase.sql', '029_product_foundation_correction.sql']) {
          db.exec(readFileSync(join(root, 'database', 'migrations', migrationFile), 'utf8'));
        }
      }
      const migration = db.prepare('SELECT version FROM schema_migrations WHERE version = ?').get('029_product_foundation_correction');
      databaseDiagnostics.migration = migration?.version === '029_product_foundation_correction';
      databaseDiagnostics.migrationVersion = migration?.version || null;
      if (!databaseDiagnostics.migration) throw new Error('Migration 029_product_foundation_correction was not recorded.');
      ...
      databaseDiagnostics.migrationVersion = '029_product_foundation_correction';
...
function productFoundation(id) {
  ...
  const variantAxes=db.prepare(`SELECT pva.attribute_id,pva.sort_order,pad.name,pad.name_en,pad.name_zh,pad.code,pad.data_type,pad.unit
    FROM product_variant_axes pva JOIN product_attribute_definitions pad ON pad.id=pva.attribute_id WHERE pva.product_id=? ORDER BY pva.sort_order,pad.name`).all(id);
  ...
  return {
    ...
    variantAxes,
    variantAxisWarning:variantAxes.length>3?'This product has many core differences. Confirm whether it should be split into separate products.':null,
...
function attributeCategoryConfigs(attributeId){return db.prepare(`SELECT category_id,required,filterable,show_on_product,show_on_quote,show_on_pi,internal_only,can_be_variant_axis,sort_order,default_unit
...
function syncAttributeMaster(attributeId,body){
  if(Array.isArray(body.category_configs)||Array.isArray(body.category_ids)){
    ...
    const insert=db.prepare(`INSERT INTO product_attribute_category_links(attribute_id,category_id,required,filterable,show_on_product,show_on_quote,show_on_pi,internal_only,can_be_variant_axis,sort_order,default_unit)
      VALUES(?,?,?,?,?,?,?,?,?,?,?)`);
...
async createProductVariant(req,res,productId){const user=currentUser(req);if(!['Admin','Owner','Designer','VA'].includes(user?.role))return json(res,user?403:401,{error:'Product editing is not allowed.'});if(!db.prepare('SELECT id FROM products WHERE id=?').get(productId))return json(res,404,{error:'Product not found.'});const b=await readJson(req),status=String(b.status||'Active');if(!masterValues('Product Status Options',variantStatuses).includes(status))return json(res,400,{error:'Variant status is not supported.'});const values=variantFieldValues(b,{},user);values[0]=requiredText(values[0],'Variant name');db.exec('BEGIN IMMEDIATE');try{const row=db.prepare('INSERT INTO product_variants(product_id,variant_name,variant_sku,dimensions,material,finish,color,reference_price,cost_price,moq,lead_time_days,cbm,gross_weight_kg,net_weight_kg,packing_info,default_supplier,supplier_sku,supplier_cost,supplier_lead_time_days,supplier_moq,supplier_notes,status,sort_order,last_updated_by) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) RETURNING *').get(productId,...values,status,Number(b.sort_order||0),user.id);syncVariantOptionValues(productId,row.id,b.option_values);db.exec('COMMIT');return json(res,201,{variant:redactSensitiveProductData({...row,option_values:b.option_values||[]},user)})}catch(error){if(db.isTransaction)db.exec('ROLLBACK');throw error}},
  ...
  async updateProductFoundation(req,res,id){const user=currentUser(req);if(!['Admin','Owner','Designer','VA'].includes(user?.role))return json(res,user?403:401,{error:'Product editing is not allowed.'});const product=db.prepare('SELECT id,category_id FROM products WHERE id=?').get(id);if(!product)return json(res,404,{error:'Product not found.'});const b=await readJson(req);db.exec('BEGIN IMMEDIATE');try{
    ...
    if(Array.isArray(b.variant_axis_attribute_ids)){const allowedAxes=new Set(db.prepare('SELECT attribute_id FROM product_attribute_category_links WHERE category_id=? AND can_be_variant_axis=1').all(product.category_id).map(row=>Number(row.attribute_id))),axisIds=normalizedIds(b.variant_axis_attribute_ids);for(const attributeId of axisIds)if(!allowedAxes.has(attributeId)){const error=new Error('Variant Axis must be enabled for the selected product category.');error.status=400;throw error}db.prepare('DELETE FROM product_variant_axes WHERE product_id=?').run(id);const insertAxis=db.prepare('INSERT INTO product_variant_axes(product_id,attribute_id,sort_order) VALUES(?,?,?)');axisIds.forEach((attributeId,index)=>insertAxis.run(id,attributeId,index))}
    if(Array.isArray(b.related_product_ids)||Array.isArray(b.frequently_bought_together_ids)){db.prepare('DELETE FROM product_foundation_relationships WHERE source_product_id=?').run(id);const insertRelationship=db.prepare('INSERT INTO product_foundation_relationships(source_product_id,target_product_id,relationship_type,sort_order) VALUES(?,?,?,?)');for(const [type,ids] of [['related',b.related_product_ids||[]],['frequently_bought_together',b.frequently_bought_together_ids||[]]])ids.forEach((target,index)=>{if(Number(target)!==id)insertRelationship.run(id,Number(target),type,index)})}
